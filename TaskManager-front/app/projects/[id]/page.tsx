...
export default function ProjectDetailPage() {
  ...
  const handleAddMember = async () => {
    if (memberForm.createNew) {
      ...
      try {
        const newUser = await addEmployeeFromContext({
        ...
        if (newUser) {
          await addProjectMember(project.id, { userId: newUser.id, role: memberForm.role });
        }
        setAddMemberOpen(false);
        setMemberForm({ existingUserId: '', role: 'membre', createNew: false, name: '', email: '', password: '', specialty: 'Designer' });
        toast({ title: 'Membre ajouté', description: `${memberForm.name} a rejoint l'équipe du projet.` });
      } catch (err) {
        toast({ title: 'Échec de l\'ajout', description: err instanceof Error ? err.message : 'Une erreur est survenue', variant: 'destructive' });
      }
      return;
    }

    if (!memberForm.existingUserId) return;
    const memberName = getUser(users, memberForm.existingUserId)?.name;
    try {
      await addProjectMember(project.id, { userId: memberForm.existingUserId, role: memberForm.role });
      setAddMemberOpen(false);
      setMemberForm({ existingUserId: '', role: 'membre', createNew: false, name: '', email: '', password: '', specialty: 'Designer' });
      toast({ title: 'Membre ajouté', description: `${memberName} a rejoint l'équipe du projet.` });
    } catch (err) {
      toast({ title: 'Échec de l\'ajout', description: err instanceof Error ? err.message : 'Une erreur est survenue', variant: 'destructive' });
    }
  };
