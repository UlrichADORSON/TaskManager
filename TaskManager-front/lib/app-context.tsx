...
import { loginRequest, fetchMe, clearTokens, getToken, apiFetch, mapBackendUserToFrontUser, fetchUsers, fetchProjects } from '@/lib/api';
...
interface AppState {
  ...
  addEmployee: (data: { name: string; email: string; password: string; phone: string; role: 'chef_de_projet' | 'membre'; memberSpecialty?: MemberSpecialty; avatarUrl?: string; company?: string; address?: string; bio?: string }) => Promise<void>;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
        ...
        if (token) {
          ...
          if (!cancelled && user) {
            ...
            setUsers((prev) => {
              ...
              const updated = { ...(user as User), active: true, lastActive: new Date().toISOString() };
              return exists
  ...
  // ---- Project actions
  const submitProject: AppState['submitProject'] = useCallback(async (data) => {
    ...
    setNotifications((prev) => [
      { id: `n-${Date.now()}`, userId: 'u-admin-1', type: 'project_submitted', title: 'Nouveau projet soumis', message: `« ${data.title} » attend validation.`, projectId: optimisticProject.id, read: false, createdAt: new Date().toISOString() },
      ...prev,
    ]);
    ...
    try {
      ...
      if (!res.ok) {
        const error = await res.json().catch(() => ({ message: 'Échec de la création' }));
      ...
      setProjects((prev) => prev.map((p) =>
        p.id === optimisticProject.id
          ? {
              ...
              members: created.members ?? p.members,
            }
          : p
      ));
    } catch (err) {
  ...
  // ---- Member management
  const addProjectMember: AppState['addProjectMember'] = useCallback(async (projectId, data) => {
    // Optimistic update
    setProjects((prev) => prev.map((p) => {
    ...
    try {
      const res = await apiFetch(`/api/projects/${parseInt(projectId, 10)}/members`, {
        method: 'POST',
        body: JSON.stringify({ userId: parseInt(data.userId, 10), role: data.role }),
      });

      if (!res.ok) {
        const error = await res.json().catch(() => ({ message: 'Échec de l\'ajout' }));
      ...
      setProjects((prev) => prev.map((p) =>
        p.id === projectId ? { ...p, members: result.members } : p
      ));
      ...
      // Rollback on error
      setProjects((prev) => prev.map((p) =>
        p.id !== projectId ? p : { ...p, members: p.members.filter((m) => m.userId !== data.userId) }
      ));
      throw err;
    }
  }, []);
  ...
  const removeProjectMember: AppState['removeProjectMember'] = useCallback(async (projectId, userId) => {
    const previousMembers = projects.find((p) => p.id === projectId)?.members ?? [];
    ...
    try {
      const res = await apiFetch(`/api/projects/${parseInt(projectId, 10)}/members/${parseInt(userId, 10)}`, {
        method: 'DELETE',
      });

      if (!res.ok) {
        const error = await res.json().catch(() => ({ message: 'Échec de la suppression' }));
      ...
      // Rollback on error
      setProjects((prev) => prev.map((p) =>
        p.id !== projectId ? p : { ...p, members: previousMembers }
      ));
      throw err;
    }
  }, [projects]);
  ...
  const addEmployee: AppState['addEmployee'] = useCallback(async (data) => {
    ...
    const optimisticUser: User = {
      ...
      avatarUrl: data.avatarUrl ?? `https://i.pravatar.cc/150?u=${encodeURIComponent(data.email)}`,
    ...
    try {
      ...
      if (!res.ok) {
        const error = await res.json().catch(() => ({ message: 'Échec de la création utilisateur' }));
        throw new Error(error.message || 'Échec de la création utilisateur');
      }
      const created = await res.json();
      const realUser = mapBackendUserToFrontUser(created) as User;
      setUsers((prev) => prev.map((u) => (u.id === tempId ? { ...realUser, password: data.password } : u)));
      return realUser;
    } catch (err) {
      // en cas d'echec reseau, on retire l'utilisateur optimiste
  ...
  const updateUser: AppState['updateUser'] = useCallback((userId, data) => {
    ...
    (async () => {
      try {
        ...
        // echec silencieux : l'etat local reste tel quel, a rafraichir manuellement si besoin
      }
    })();
  }, [currentUser]);
